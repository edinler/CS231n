...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        self.params[f"W{self.num_layers}"] = np.random.normal(0.0, weight_scale, (input_dim, num_classes))
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          out, cache[f"affine{i+1}"] = affine_forward(X, self.params[f"W{i+1}"], self.params[f"b{i+1}"])
          if self.normalization == "batchnorm":
        ...
        for i in reversed(range(self.num_layers-1)):
          ...
          if self.normalization == "batchnorm": 
            dout, grads[f"gamma{i+1}"], grads[f"beta{i+1}"] = batchnorm_backward(dout, cache[f"batchnorm{i+1}"])
