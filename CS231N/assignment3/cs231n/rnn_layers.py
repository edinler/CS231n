...
def rnn_step_backward(dnext_h, cache):
    ...
    da, dx, dWx, dWh, db = np.zeros_like(a), np.zeros_like(x), np.zeros_like(Wx), np.zeros_like(Wh), np.zeros_like(a)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(x.shape[1]):
      next_h, cache_new = rnn_step_forward(x[:, i, :] , h0, Wx, Wh, b)
      h.append(next_h)
      h0 = next_h
...
def rnn_backward(dh, cache):
    ...
    x, prev_h, Wx, Wh, b, a, next_h = cache[0]
    D = x.shape[1]
    N, T, H = dh.shape
    dWx = np.zeros((D, H))
    dWh = np.zeros((H, H))
    ...
    for i in reversed(range(dh.shape[1])):
      dx, dprev_h, dWx_new, dWh_new, db_new = rnn_step_backward(dh[:, i, :]+dprev_h, cache[i])
      dx0.append(dx)
      dWx += dWx_new
      dWh += dWh_new
      db += db_new
