...
def sample_noise(batch_size, dim, seed=None):
    ...
    out = torch.rand(batch_size,dim) - torch.rand(batch_size, dim)
...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss_real = bce_loss(logits_real, target_real.squeeze())
    loss_fake = bce_loss(logits_fake, target_fake.squeeze())
...
def generator_loss(logits_fake):
    ...
    loss = bce_loss(logits_fake, target_fake.squeeze())
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    target_real = torch.ones_like(scores_real)
    target_fake = torch.zeros_like(scores_fake)
    loss_real = (target_real - scores_real)**2
    loss_fake = (target_fake - scores_fake)**2
...
def ls_generator_loss(scores_fake):
    ...
    target_fake = torch.ones_like(scores_fake)
    loss_fake = (scores_fake - target_fake)**2
    loss = loss_fake.mean()/2
